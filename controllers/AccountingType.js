...
module.exports.getbyid = async (req, res) => {
  try {
    ...
    const data = await Model.findByPk(from.AccountingTypeID);
    res.status(201).json({
      data: data,
...
module.exports.getByOne = async (req, res) => {
  try {
    ...
    const data = await Model.findOne({
      where: {
        AccountTypeName: from.AccountTypeName,
      },
    });
...
module.exports.update = async (req, res) => {
  try {
    ...
    const data1 = {
      AccountTypeName: from.AccountTypeName,
      AccountTypeCode: from.AccountTypeCode,
    };
